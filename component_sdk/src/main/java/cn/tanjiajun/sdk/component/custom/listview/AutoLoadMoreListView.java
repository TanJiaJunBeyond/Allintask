package cn.tanjiajun.sdk.component.custom.listview;
...
import cn.tanjiajun.sdk.component.custom.listview.xlistview.XListViewFooter;
import cn.tanjiajun.sdk.component.custom.listview.xlistview.XListViewHeader;
...
public class AutoLoadMoreListView extends ListView implements OnScrollListener {
    ...
    // -- fading header view container
    private LinearLayout mFadingHeaderContainer;
    ...
    private void initWithContext(Context context) {
        ...
        // init fading header view;
        mFadingHeaderContainer = new LinearLayout(context);
        addHeaderView(mFadingHeaderContainer);
    ...
    public void setFadingHeaderView(View view) {
        if (null != mFadingHeaderContainer && null != view) {
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT);
            mFadingHeaderContainer.addView(view, params);
        }
    }

    public View getFadingHeaderView() {
        View view = null;
        if (null != mFadingHeaderContainer) {
            int childCounts = mFadingHeaderContainer.getChildCount();
            if (childCounts >= 1) {
                view = mFadingHeaderContainer.getChildAt(0);
            }
        }
        return view;
    }

    public void removeFadingHeaderView() {
        if (null != mFadingHeaderContainer) {
            mFadingHeaderContainer.removeAllViews();
        }
    }

    private boolean isFadingHeaderViewFullyDisplay() {
        boolean isFullyDisplay = false;
        if (null != mFadingHeaderContainer) {
            float offsetY = mFadingHeaderContainer.getY();
            if (0.0f == offsetY) {
                isFullyDisplay = true;
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mLastY = ev.getRawY();
                if (isFadingHeaderViewFullyDisplay() && mEnablePullRefresh && getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
