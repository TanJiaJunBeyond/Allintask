...
public class EMChatDatabaseManager {
    static private EMChatDatabaseManager dbMgr = new EMChatDatabaseManager();
    private EMDbOpenHelper dbHelper;
    
    private EMChatDatabaseManager(){
        dbHelper = EMDbOpenHelper.getInstance(AllintaskApplication.getInstance().getApplicationContext());
    }
    
    public static synchronized EMChatDatabaseManager getInstance(){
        if(dbMgr == null){
            dbMgr = new EMChatDatabaseManager();
        }
        return dbMgr;
    }
    ...
    synchronized public Map<String, EaseUser> getContactList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (username.equals(CommonConstant.NEW_FRIENDS_USERNAME) || username.equals(CommonConstant.GROUP_USERNAME)
                        || username.equals(CommonConstant.CHAT_ROOM)|| username.equals(CommonConstant.CHAT_ROBOT)) {
