...
public class EaseAtMessageHelper {
    ...
    public void parseMessages(List<EMMessage> messages) {
        ...
        for(EMMessage msg : msgs){
            if(msg.getChatType() == ChatType.GroupChat){
                ...
                try {
                    JSONArray jsonArray = msg.getJSONArrayAttribute(CommonConstant.MESSAGE_ATTR_AT_MSG);
                    ...
                    String usernameStr = msg.getStringAttribute(CommonConstant.MESSAGE_ATTR_AT_MSG, null);
                    if(usernameStr != null){
                        if(usernameStr.toUpperCase().equals(CommonConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)){
                            if(!atMeGroupList.contains(groupId)){
                                atMeGroupList.add(groupId);
                            }
                        }
    ...
    public boolean isAtMeMsg(EMMessage message){
        ...
        if(user != null){
            try {
                JSONArray jsonArray = message.getJSONArrayAttribute(CommonConstant.MESSAGE_ATTR_AT_MSG);
                ...
                String atUsername = message.getStringAttribute(CommonConstant.MESSAGE_ATTR_AT_MSG, null);
                if(atUsername != null){
                    if(atUsername.toUpperCase().equals(CommonConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)){
                        return true;
                    }
                }
                return  false;
