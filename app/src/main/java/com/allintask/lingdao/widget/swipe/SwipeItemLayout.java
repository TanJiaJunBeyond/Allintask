...
public class SwipeItemLayout extends RelativeLayout {
    ...
    private void initAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.SwipeItemLayout_sil_swipeDirection) {
            ...
            if (mSpringDistance < 0)
                throw new IllegalStateException("sil_springDistance不能小于0");
    ...
    private GestureDetector.SimpleOnGestureListener mSimpleOnGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        @Override
        public boolean onSingleTapConfirmed(MotionEvent e) {// 2
            setPressed(false);
            return isClosed() && performClick();
        }

        @Override
        public boolean onSingleTapUp(MotionEvent e) {  // 1
            if (isClosed()) {
                setPressed(true);
                return true;
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mSwipeDirection == SwipeDirection.Left) {   // 向左滑动
