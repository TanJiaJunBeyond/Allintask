...
/**
 * Created by zhengwenjie on 2015/12/30 0030.
 * 流式布局
 */
public class FlowLayout extends ViewGroup {
    ...
    private int maxRow = 100;//最大行数

    public void setMaxRow(int maxRow) {
        this.maxRow = maxRow;
    }

    public void setmVerticalChildGap(int mVerticalChildGap) {
        this.mVerticalChildGap = mVerticalChildGap;
    }
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.FlowLayout_fl_horizontalChildGap) {
             ...
             * 如果是px,则不乘;两个函数的区别是一个返回float,一个返回int. getDimensionPixelSize则不管写的是dp还是sp还是px,
             * 都会乘以denstiy.
             */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < rowCount && i < maxRow; i++) {
        ...
        int paddHeight = 0;
        if (rowCount > maxRow) {
            paddHeight = (rowCount - maxRow) * mVerticalChildGap;
        }
        setMeasuredDimension(sizeWidth, modeHeight == MeasureSpec.EXACTLY ? sizeHeight : height +
                getPaddingTop() + getPaddingBottom() - paddHeight);
    }
    ...
    private class Row {
        ...
        public void layout(boolean isNeedSplit, int top) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (isNeedSplit) {
                    ...
                    if (splitWidth > 0) {
                        ...
                        int widthMeasureSpec = MeasureSpec.makeMeasureSpec(childWidth,
                                MeasureSpec.EXACTLY);
                        int heightMeasureSpec = MeasureSpec.makeMeasureSpec(childHeight,
                                MeasureSpec.EXACTLY);
                ...
                view.layout(left, top + topOffset, left + childWidth, top + topOffset +
                        childHeight);
